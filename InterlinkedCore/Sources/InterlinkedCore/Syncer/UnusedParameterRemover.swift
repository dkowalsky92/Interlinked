...
class UnusedParameterRemover {
    func removeUnusedParameters(definitions: DependencyDefinitions) -> DependencyDefinitions {
        ...
        for parameter in definitions.parameters {
            ...
            let isStandardParameterAssignment = definitions.scope.containsStandardParameterAssignment(forIdentifier: parameter.name)
            if (isMatchingVariable && isStandardParameterAssignment) || (isUsedInCodeBlockItems && !isStandardParameterAssignment) {
